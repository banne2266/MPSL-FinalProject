...
#define PWM_HI (57) // dutycycle 64%  total 89
#define PWM_LO (28) // dutycycle 32%  total 89
...
void led_set_pos(int pos) {
	for(int i = 0; i < 6; i++){
		for(int j = 0; j < 22; j++){
			if(i == pos){
				led_set_RGB(i*22+j, 0, 0, 255);
			}
			else{
				led_set_RGB(i*22+j, 0, 0, 0);
			}
		}
	}
}
